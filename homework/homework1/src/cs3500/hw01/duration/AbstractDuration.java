    ...
    for (int i = 0; i < template.length(); i++) {
      if (template.charAt(i) == '%') {
        if (template.length() == 1) {
          throw new IllegalArgumentException("Cannot have a template with a single %");
        }
        switch (template.charAt(i + 1)) {
          case 't':
            formattedString += total;
            break;
          case 'h':
          ...
          case '%':
            ...
            break;
          default:
            throw new IllegalArgumentException("Template contains an invalid format specifier.");
        }
        i += 1;
      } else {
        if (i == template.length() - 1 && template.charAt(i) == '%') {
          throw new IllegalArgumentException("Template cannot end with an unaccompanied %");
        }
        formattedString += template.charAt(i);
      }
    }
